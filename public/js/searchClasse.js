            ...
            if (data.length > 0) {
                data.forEach(function(classData) {
                    ...
                    <div class="bg-white py-4 px-3" style="background: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('photos/classe2.jpg') no-repeat center; background-size: cover;">
                            <h1 class="text-3xl text-white text-center mb-2 font-bold font-mono">${classData.name}</h1>
                            <div class="flex justify-between">
                                <a href="/classes/${classData.id}/edit">
                                    <img src="/photos/update.png" class="h-6" alt="">
                                </a>
                                <a href="#" onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this subject?')) { document.getElementById('delete-form-${classData.id}').submit(); }"><img src="/photos/delete.png" class="h-6" alt=""></a>
                                <form id="delete-form-${classData.id}" action="/classes/${classData.id}" method="POST" style="display: none;">
